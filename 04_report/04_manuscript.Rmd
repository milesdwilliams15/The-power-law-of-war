...
cited above. Three statistical models are tested using these methods:
the power-law, the log-normal, and the inverse Burr. Model validation
tests fail to reject each of these models; however, this varies by how
war size is operationalized. Three ways war size is measured in the
literature are considered: total battle deaths, battle deaths per global
population, and battle deaths per the combined populations of the
countries directly involved in a war. When war size is measured in total
deaths, the power-law cannot be rejected and the inverse Burr only
barely escapes rejection. When war size is normalized by global
population, both the power-law and inverse Burr perform well. Finally,
when war size is normalized by belligerent country population, all
models cannot be rejected and the sign on pairwise likelihood ratio
tests favor the log-normal model.
...
they may open themselves to criticism that they have selected the
statistical model that provides the results they were looking for.
