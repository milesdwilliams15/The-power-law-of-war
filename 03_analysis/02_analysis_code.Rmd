...
p1 <- ggplot(Data) +
  ...
  labs(
    ...
    y = "log[Pr(X > x)]",
    subtitle = "Power-law fit to war deaths"
  )
obs.x <- Data$deaths_pc
new.x <- exp(seq(min(log(obs.x)), max(log(obs.x)), len = 200))
mu.x  <- mean(log(obs.x))
sd.x  <- sd(log(obs.x))
dens <- 1 - pnorm(q = log(new.x), mean = mu.x, sd = sd.x)
p2 <- ggplot() +
  geom_point(
    data = Data,
    aes(x = log(deaths_pc),
  ...
  labs(
    x = "War Deaths per Capita (log)",
    y = "log[Pr(X > x)]",
    subtitle = "Log-normal fit to war deaths"
  )
p1 + p2 +
  plot_annotation(
    title = "Power-law or Log-normal?"
  )
save_fig("fig1.png")
```
...
ln.fit <- ln(Data$deaths_pc)
pl.fit$estimate[1] <- (100 * pl.fit$estimate[1])
pl.fit$bootstrap$xmin <- (100 * pl.fit$bootstrap$xmin)
ln.fit$estimate[1] <- 100 * exp(ln.fit$estimate[1])
ln.fit$bootstrap$mu <- 100 * exp(ln.fit$bootstrap$mu)
library(kableExtra)
list(pl.fit, ln.fit) %>%
  map_dfr(
    ~ tibble(
  ...
  mutate(
    ` ` = c("x-min (% of population)", "beta", "mu", "sigma")
  ) %>%
  kable(
    "latex",
...
dt.pl <- tibble(
  ...
  pmin = PXx(Data$deaths_pc)[Data$deaths_pc==(pl.fit$estimate[1]/100)],
  pl = (pmin * ((x/(pl.fit$estimate[1]/100))^(-pl.fit$estimate[2] + 1)) - p)/p,
  ll = ((1 - pnorm(log(x), log(ln.fit$estimate[1]/100), ln.fit$estimate[2])) - p)/p
)
...
```{r}
data <- round(100000000 * Data$deaths_pc)
...
# sim some pl data based on pl parameter fits
replicate(
  200,
  {
    sim.x <- rpldis(n = 95, xmin = pl.fit$estimate[1]*100, alpha = pl.fit$estimate[2])
    list(shapiro.test(sim.x))
  }
) -> out.sim
map_dfr(
  out.sim,
  ~ tibble(p.value = .x$p.value)
) -> p.sim
hist(p.sim$p.value)
c(median = median(p.sim$p.value),
  min. = min(p.sim$p.value),
  max. = max(p.sim$p.value))
```
...
dt.pl <- tibble(
  ...
  pmin = PXx(Data$deaths_pc)[Data$deaths_pc==(pl.fit$estimate[1]/100)],
  pl = pmin * ((x/(pl.fit$estimate[1]/100))^(-pl.fit$estimate[2] + 1)),
  ll = 1 - pnorm(log(x), log(ln.fit$estimate[1]/100), ln.fit$estimate[2])
)
...
1:nrow(pl.fit$bootstrap) %>%
  map_dfr(
    ~ {
      tibble(
        x = 1/10,
        pl = ((x/(pl.fit$bootstrap$xmin[.x]/100))^(-pl.fit$bootstrap$beta[.x]+1)),
        ll = 1 - pnorm(log(x), log(ln.fit$bootstrap$mu[.x]/100),
                       ln.fit$bootstrap$sigma[.x])
      ) 
    }
  ) -> boot_preds
